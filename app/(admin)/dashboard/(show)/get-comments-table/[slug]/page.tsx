...
function page() {
    ...
    const getCommentsHandler = async (slug: string) => {
        setLoading(true)
        const {comments} = await getComments(slug, 10)
        // @ts-ignore
                    ...
                    <TableBody>
                        {comments?.map((comment: any, index) => {
                            async function onSubmitHandler() {
                                ...
                                const response = await fetch("/api/is_published/is_published_comments", {
                                    method: "PATCH",
                                    headers: {
                                    ...
                                    <TableCell className="text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end"
                                                ...
                                                <DropdownMenuLabel className='text-dark300_light700'>Actions</DropdownMenuLabel>
                                                ...
                                                <DropdownMenuItem>
                                                    <div
                                                        ...
                                                        onClick={async () => {
                                                            await deleteSingleComment(comment.id);
                                                            router.refresh();
                                                        }}>
                                                        <TrashIcon/>
                                                        Delete
